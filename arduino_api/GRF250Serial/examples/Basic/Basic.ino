...
void setup() {
  ...
  SERIAL_MONITOR.print("Product name: ");
  SERIAL_MONITOR.println(product_info.product_name);
  SERIAL_MONITOR.print("Hardware version: ");
  SERIAL_MONITOR.println(product_info.hardware_version);
  SERIAL_MONITOR.print("Firmware version: ");
  SERIAL_MONITOR.print(product_info.firmware_version.major);
  SERIAL_MONITOR.print(".");
  SERIAL_MONITOR.print(product_info.firmware_version.minor);
  SERIAL_MONITOR.print(".");
  SERIAL_MONITOR.println(product_info.firmware_version.patch);
  SERIAL_MONITOR.print("Serial number: ");
  SERIAL_MONITOR.println(product_info.serial_number);
  ...
  SERIAL_MONITOR.print("Polled distance: ");
  SERIAL_MONITOR.print(distance_data.first_return_raw_mm);
  SERIAL_MONITOR.println(" mm");
}
...
void loop() {
  ...
  for (int i = 0; i < 10; ++i) {
    ...
    if (result == LW_RESULT_SUCCESS) {
      SERIAL_MONITOR.print("Streamed distance: ");
      SERIAL_MONITOR.print(distance_data.first_return_raw_mm);
      SERIAL_MONITOR.println(" mm");
